...
class stimulationFES {
    constructor(channels, joint, index, mode, pw, current, ccl_update, ccl_zero) {
        ...
        this.ccl_update = ccl_update;
        this.ccl_zero = ccl_zero;
    }
}
...
io.on('connection', (socket) => {
    ...
    socket.on('FES:ask_serial_port', function() {
        ...
        exec('find /sys/bus/usb/devices/usb*/ -name tty', (err, stdout, stderr) => {
          ...
          if (command_out != '') {
            for (var i = 0; i < command_out.length - 6; i++) {
              if (command_out.substr(i,6) == "ttyUSB") {
                serialPorts.push(command_out.substr(i,7))
              }
            }
            console.log(serialPorts);
          } else {
            console.log("No USB ports connected");
          }
          socket.emit('FES:get_serial_port', {
              serialPorts: serialPorts
          })
...
function configFES (channels_Stim, current, pw ,main_freq, group_time, mode) {
    ...
    return [ccl_init, ccl_update, ccl_stop];
}
